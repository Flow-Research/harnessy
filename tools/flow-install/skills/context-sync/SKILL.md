...
11. Generate commit messages from the actual staged eligible change set and repository context.
12. After `push`, automatically create or reuse the PR for the selected target branch without rebasing onto `dev` as part of the normal push flow.
13. After PR creation or reuse, start or resume a bounded autonomous PR-resolution loop.
14. The PR-resolution loop must keep ownership until the PR reaches a terminal success state or an explicit safe escalation state. No failed PR may be left unattended.
15. Use a persistent run ledger inside `.git/context-sync/` for `push` so retries, watcher restarts, and follow-up fixes remain deterministic.
